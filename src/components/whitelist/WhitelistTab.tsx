...
import { validateMacAddress } from '../../utils';

interface WhitelistTabProps {
  whitelist: WhitelistItem[];
  onSave: (name: string, macs: string[]) => Promise<any>;
  onUpdate: (id: number, name: string, macs: string[]) => Promise<any>;
  onDelete: (id: number) => Promise<any>;
}

/**
 * Whitelist management tab component
 */
export function WhitelistTab({ whitelist, onSave, onUpdate, onDelete }: WhitelistTabProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const invalidMacs = macs.filter(mac => !validateMacAddress(mac));
    ...
    try {
      if (editingItem) {
        await onUpdate(editingItem.id, formData.name, macs);
      } else {
        await onSave(formData.name, macs);
      }
